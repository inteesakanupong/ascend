...
function renderWeigh() {
  ...
  const log = getDailyLog(today) || { date: today };
  ...
  if (trendMetaEl) {
    ...
    if (rp.weeklyKgLoss != null) {
      ...
      trendMetaEl.innerHTML = `<span style="color:${color};font-weight:700;">${rate >= 0 ? "down " : "up "}${Math.abs(rate)}kg/wk</span>`;
    } else {
      trendMetaEl.textContent = `${allLogs.length} ENTRIES`;
    }
  }
  $("#weigh-phase").innerHTML = `<span class="phase-tag phase-${phase.toLowerCase()}">${phase}</span> · ${adjKcal} KCAL TARGET${targets.restDay ? " · REST" : ""}${deltaNote}`;
...
function renderRecentWeighIns() {
  ...
  if (recent.length === 0) {
    wrap.innerHTML = `<div class="empty"><div class="glyph">o</div><div class="label">No daily logs yet</div></div>`;
    return;
  }
