...
function nutritionFiniteNumber(value, fallback = null) {
...
function nutritionStateLatestWeight(state) {
  const logs = (Array.isArray(state?.dailyLogs) ? state.dailyLogs : [])
    .filter(d => d && Number.isFinite(Number(d.weight)))
    .sort((a, b) => String(a.date || "").localeCompare(String(b.date || "")));
  return nutritionFiniteNumber(logs[logs.length - 1]?.weight, nutritionFiniteNumber(state.profile?.bodyweight, 80));
}

function nutritionCalorieFloor(state, weight) {
  const sexFloor = state.profile?.sex === "female" ? 1200 : 1500;
  return Math.max(sexFloor, Math.round(nutritionFiniteNumber(weight, 80) * 22));
}
...
function initialAdaptiveCalories(state, weight) {
  ...
  let tdee = null;
  try {
    tdee = typeof computeTDEE === "function"
      ? computeTDEE(p.sex, weight, p.height, p.age, p.activityLevel)
      : null;
  } catch (error) {
    console.warn("Adaptive calorie estimate fell back to bodyweight", error);
  }
...
function migrateNutritionCoachState(state) {
  ...
  state.cut = state.cut && typeof state.cut === "object" ? state.cut : {};
  state.dailyLogs = Array.isArray(state.dailyLogs) ? state.dailyLogs : [];
  state.dietProfile = state.dietProfile && typeof state.dietProfile === "object" ? state.dietProfile : {};

  const existing = state.nutritionCoach && typeof state.nutritionCoach === "object"
    ? state.nutritionCoach
    : null;
  const existingRecovery = existing?.recovery && typeof existing.recovery === "object"
    ? existing.recovery
    : {};
  state.nutritionCoach = {
    ...nutritionCoachTemplate(),
    ...(existing || {}),
    recovery: { ...nutritionCoachTemplate().recovery, ...existingRecovery },
    reviews: Array.isArray(existing?.reviews) ? existing.reviews : [],
  };
  const normalizedWeight = nutritionStateLatestWeight(state);
  const fallbackKcal = nutritionFiniteNumber(state.cut.p1Kcal, initialAdaptiveCalories(state, normalizedWeight));
  state.nutritionCoach.currentKcal = nutritionFiniteNumber(state.nutritionCoach.currentKcal, fallbackKcal);
  state.nutritionCoach.startingKcal = nutritionFiniteNumber(state.nutritionCoach.startingKcal, state.nutritionCoach.currentKcal);
  state.nutritionCoach.startWeight = nutritionFiniteNumber(state.nutritionCoach.startWeight, normalizedWeight);
  state.nutritionCoach.checkpointWeight = nutritionFiniteNumber(state.nutritionCoach.checkpointWeight, null);
  state.nutritionCoach.finalGoalWeight = nutritionFiniteNumber(state.nutritionCoach.finalGoalWeight, null);
  state.nutritionCoach.weeklyLossTargetKg = nutritionFiniteNumber(state.nutritionCoach.weeklyLossTargetKg, 0.6);
  state.nutritionCoach.adjustmentStepKcal = nutritionFiniteNumber(state.nutritionCoach.adjustmentStepKcal, 100);
  state.nutritionCoach.minCompleteDays = Math.max(1, Math.round(nutritionFiniteNumber(state.nutritionCoach.minCompleteDays, 5)));
...
function nutritionDateOffset(iso, days) {
  ...
  if (!(d instanceof Date) || Number.isNaN(d.getTime())) return todayISO();
...
function nutritionLogsInWindow(endDate, days) {
  ...
  return (Array.isArray(STATE.dailyLogs) ? STATE.dailyLogs : [])
    .filter(d => d && typeof d.date === "string" && d.date >= start && d.date <= endDate);
}
...
function adaptiveNutritionStatus(date = todayISO()) {
  ...
  const onPlan = d => {
    ...
    const runAdjustment = typeof activityFuelingAdjustmentForDate === "function"
      ? activityFuelingAdjustmentForDate(d.date).adjustment
      : typeof runFuelingAdjustmentForDate === "function"
  ...
  const weeklyBudget = Array.from({ length: 7 }, (_, i) => {
    ...
    const runAdjustment = typeof activityFuelingAdjustmentForDate === "function"
      ? activityFuelingAdjustmentForDate(day).adjustment
      : typeof runFuelingAdjustmentForDate === "function"
